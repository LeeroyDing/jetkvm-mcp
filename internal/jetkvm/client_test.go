...
func TestStatusMetadataCannotEchoConfiguredCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:       "password",
			credential: "7passwordEcho",
			password:   "7passwordEcho",
			credentials: Credentials{
				Password: NewSecret("7passwordEcho"),
			},
		},
		{
			name:       "auth token",
			credential: "8tokenEcho",
			credentials: Credentials{
				AuthToken: NewSecret("8tokenEcho"),
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			fd := startFakeDevice(t, fakeDeviceOptions{
				...
				DeviceID:      "id-" + tc.credential,
				DeviceVersion: tc.credential,
			})
			ctx := contextWithTimeout(t, 15*time.Second)
			client, err := Connect(ctx, Options{BaseURL: fd.baseURL(), Credentials: tc.credentials})
			if err != nil {
				t.Fatalf("Connect failed: %v", err)
			}
			defer client.Close(context.Background())
			...
			if strings.Contains(status.DeviceID, tc.credential) || strings.Contains(status.FirmwareVersion, tc.credential) {
				t.Fatalf("status metadata reflected configured credential: %+v", status)
			}
			if status.DeviceID != redactionPlaceholder || status.FirmwareVersion != redactionPlaceholder {
				t.Fatalf("reflected metadata = %+v, want redaction placeholders", status)
			}
		})
	}
}

func TestStatusRPCErrorCannotEchoConfiguredCredentials(t *testing.T) {
	for _, tc := range []struct {
		name        string
		credential  string
		credentials Credentials
	...
	}{
		{
			...
			password:   "hunter2",
			credentials: Credentials{
				Password: NewSecret("hunter2"),
			},
		},
		{
			name:       "auth token",
			credential: "short-token",
			credentials: Credentials{
				AuthToken: NewSecret("short-token"),
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Contains(err.Error(), tc.credential) {
				t.Fatalf("status RPC error reflected configured credential: %v", err)
			}
		})
	}
}
...
func TestCaptureScreenshotFailsWhenContextEndsDuringPNGEncode(t *testing.T) {
	fd := startFakeDevice(t, fakeDeviceOptions{})
	connectCtx := contextWithTimeout(t, 15*time.Second)
	client, err := Connect(connectCtx, Options{BaseURL: fd.baseURL()})
	if err != nil {
		t.Fatalf("Connect failed: %v", err)
	}
	defer client.Close(context.Background())
	...
	defer cancel()
	client.encode = func(w io.Writer, _ image.Image) error {
		cancel()
		_, err := w.Write([]byte("must not be returned"))
		return err
	}
	shot, err := client.CaptureScreenshot(shotCtx)
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("CaptureScreenshot error = %v, want context cancellation during encode", err)
	}
	if len(shot.PNG) != 0 {
		t.Fatal("canceled PNG encode returned partial screenshot bytes")
	}
}
